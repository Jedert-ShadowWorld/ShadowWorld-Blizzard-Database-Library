...
namespace BlizzardDatabaseLib {
    namespace Reader {
        WDC5RecordReader::WDC5RecordReader(std::shared_ptr<Stream::StreamReader> streamReader, Structures::VersionDefinition& versionDefinition, Stream::BitReader& bitReader, Structures::WDC5Header& fileHeader)
            : _bitReader(bitReader), _streamReader(streamReader), _versionDefinition(versionDefinition), _fileHeader(fileHeader)
        {

        }

        Structures::BlizzardDatabaseRow WDC5RecordReader::ReadRecord(int indexOfId, Structures::WDC5Section& section, Stream::BitReader& reader,
